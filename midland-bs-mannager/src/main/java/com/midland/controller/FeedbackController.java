...
public class FeedbackController extends BaseFilter {
	...
	@ResponseBody
	public Object batchUpdate(String ids,Feedback feedback) throws Exception {
		List<Feedback> commentList = new ArrayList<>();
		String[] ides=ids.split(",",-1);
		for (String id:ides ){
			Feedback comment1 = new Feedback();
			comment1.setId(Integer.valueOf(id));
			comment1.setIsDelete(feedback.getIsDelete());
			commentList.add(comment1);
		}
		Map<String,Object> map = new HashMap<>();
		try {
			log.debug("updateCategoryById  {}",commentList);
			feedbackServiceImpl.batchUpdate(commentList);
			map.put("state",0);
		} catch(Exception e) {
