    ...
    <section class="content">
        ...
        <form id="formId" action="${ctx}/rest/activity/saveEditActivity" enctype="multipart/form-data" method="post">
            <ul class="adminfo row">
                <li><span>活动标题：<input type="hidden" name="id" value="${activity.id}"></span><input maxlength="50"
                                                                                                   type="text"
                                                                                                   name="actiTitle"
                                                                                                   value="${activity.actiTitle}"><span
                        class="_star ">*</span></li>
                    ...
                    <div id="img" style="display:inline-block; float:left;">
                        <div class="imgbox">
                            <div class="imgnum">
                                ...
                                <img src="<c:if test='${!empty activity.acti_thumb_pic1 and activity.acti_thumb_pic1!="" }'>${activity.acti_thumb_pic1 }</c:if>  <c:if test='${empty activity.acti_thumb_pic1 or activity.acti_thumb_pic1=="" }'>${activity.acti_thumb_pic1 }${ctx}/assets/img/btn.png</c:if>"
                                     class="img1"/>
                                <img src="" class="img2"/>
                            </div>
                        </div>
                    </div>
                    <div style="font-size:12px; color:#666; height:202px; line-height:202px;">(请添加大小不超过500K、分辨率为1180 *
                        340 Pix、图片格式为JPG或PNG的图片)
                    </div>
                </li>
                ...
                <li style="overflow:hidden;">
                    ...
                    <textarea style=" float:left; width: 92%;min-height: 350px;resize:none; outline-color: #0099e0;"
                              name="actiDetails" id="myEditor" rows="" cols="">${activity.actiDetails}</textarea></li>
                <li><span>排序：</span><input maxlength="3" type="text" name="sortOrder" value="${activity.sortOrder}"
                                           onfocus="InitInput.setNumber(this,3,0,0)"><span class="_star ">*</span></li>
                <li style="display:flex;align-items:center">
                    <span>活动状态：<input name="isShow" id="isShow" type="hidden" value="${activity.isShow}"></span>
                    <input style="width: 15px;height: 15px;margin: 0px;" id="radio1" onchange="addIsShow();"
                           type="radio" name="isshow" value="1">
                    <em class="gang" style="margin-right:15px;">开放</em>
                    <input style="width: 15px;height: 15px;margin: 0px;" id="radio2" onchange="addIsShow();"
                           type="radio" name="isshow" value="0">
                    <em class="gang">关闭</em>
                    <span class="_star ">*</span>
                </li>
                <li><span>发布日期：</span><input
                        onFocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})"
                        value="${activity.addTime}" type="text" name="addTime"><span class="_star ">*</span></li>
            </ul>
            <ul class="adminfo row">
                <li>
                    <span></span>
                    <a onclick="document.getElementById('formId').submit();" target="contentF"
                       class="public_btn bg2">保存</a>
                    <a style="margin-left: 20px" href="${ctx}/rest/activity/index" target="contentF"
