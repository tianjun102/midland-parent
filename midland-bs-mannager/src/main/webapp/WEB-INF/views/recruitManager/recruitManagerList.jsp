                    ...
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <c:if test="${item.releaseStatus==0}">已上线</c:if><c:if
                                        test="${item.releaseStatus==1}">已下线</c:if>
                                </c:otherwise>
                            </c:choose>
                        ...
                        <td>
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <c:if test="${empty item.releaseStatus or item.releaseStatus==1}"><a
                                            target="contentF" title="上线" class="lineup_img"
                                            onclick="updateStatus(${item.id},${item.releaseStatus});"></a></c:if>
                                    <c:if test="${item.releaseStatus==0}"><a target="contentF" title="下线"
                                                                             class="linedown_img"
                                                                             onclick="updateStatus(${item.id},${item.releaseStatus});"></a></c:if>
...
<script type="text/javascript">
    ...
    function updateStatus(id, status) {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/rest/recruitManager/publish?releaseStatus=" + status + "&id=" + id,
            async: false, // 此处必须同步
            dataType: "json",
