    ...
    <form action="${ctx}/rest/siteProtocol/update" method="post" id="dataForm">
        <ul class="adminfo width-lg row">
            ...
            <li id="textArea" style="display: block;"><span>私隐政策：</span>
                <textarea id="myEditor" name="privacy" class="textarea-lg">${item.privacy}</textarea>
            </li>
            <li>
                <span></span>
...
<script type="text/javascript">
    ...
    function siteProtocol() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.state == 0) {
                    $("#id").val(data.item.id);
                    UE.getEditor('myEditor').setContent(data.item.privacy==null?"":data.item.privacy);
    ...
    function updateData() {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/rest/siteProtocol/update/privacy",
            async: false, // 此处必须同步
            dataType: "json",
            data: data,
            success: function (data) {
