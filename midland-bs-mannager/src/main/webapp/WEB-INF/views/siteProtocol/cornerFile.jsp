...
<section class="content" style="border:none;">
    <form action="${ctx}/rest/siteProtocol/update" method="post" id="dataForm">
        <ul class="adminfo width-lg row">
            ...
            <li id="textArea" style="display: block;"><span>备案编号：</span>
                <textarea id="myEditor" name="cornerFile" class="textarea-lg">${item.cornerFile}</textarea>
            </li>
            <li>
                <span></span>
...
<script type="text/javascript">
    ...
    function siteProtocol() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.state == 0) {
                    $("#id").val(data.item.id);
                    UE.getEditor('myEditor').setContent(data.item.cornerFile==null?"":data.item.cornerFile);
    ...
    function updateData() {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/rest/siteProtocol/update/cornerFile",
            async: false, // 此处必须同步
            dataType: "json",
            data: data,
            success: function (data) {
