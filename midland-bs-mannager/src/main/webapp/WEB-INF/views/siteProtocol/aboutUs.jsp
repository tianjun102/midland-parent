    ...
    <form action="${ctx}/rest/siteProtocol/update" method="post" id="dataForm">
        <ul class="adminfo width-lg row">
            ...
            <li id="textArea" style="display: block;"><span>关于美联：</span>
                <textarea id="myEditor" name="aboutUs" class="textarea-lg">${item.aboutUs}</textarea>
            </li>
            <li>
                <span></span>
...
<script type="text/javascript">
    ...
    function siteProtocol() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.state == 0) {
                    $("#id").val(data.item.id);
                    UE.getEditor('myEditor').setContent(data.item.aboutUs==null?"":data.item.aboutUs);
    ...
    function updateData() {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/rest/siteProtocol/update/aboutUs",
            async: false, // 此处必须同步
            dataType: "json",
            data: data,
            success: function (data) {
