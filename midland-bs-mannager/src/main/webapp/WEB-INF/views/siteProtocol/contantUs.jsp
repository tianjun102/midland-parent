    ...
    <form action="${ctx}/rest/siteProtocol/update" method="post" id="dataForm">
        <ul class="adminfo width-lg row">
            ...
            <li id="textArea" style="display: block;"><span>联系我们：</span>
                <textarea id="myEditor" name="contantUs" class="textarea-lg">${item.contantUs}</textarea>
            </li>
            <li>
                <span></span>
...
<script type="text/javascript">
    ...
    function siteProtocol() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.state == 0) {
                    $("#id").val(data.item.id);
                    UE.getEditor('myEditor').setContent(data.item.contantUs==null?"":data.item.contantUs);
    ...
    function updateData() {
        ...
        $.ajax({
            type: "post",
            url: "${ctx}/rest/siteProtocol/update/contantUs",
            async: false, // 此处必须同步
            dataType: "json",
            data: data,
            success: function (data) {
