...
<html lang="en" class="no-js">
	...
	<div class="box">
		...
		<form id="formId" action="${ctx}/rest/ad/saveEditAd" method="post" enctype="multipart/form-data" method="post">
				<input type="hidden" name="catId" value="${adProd.catId}" id="cat_Id" >
				<input type="hidden" name="id" value="${ad.adId}"  >
				<input type="hidden" name="isAll" value="${adProd.isAll}">
				<input type="hidden" name="ad_thumb_pic1" value="${ad.ad_thumb_pic1}">
				<input type="hidden" name="ad_thumb_pic2" value="${ad.ad_thumb_pic2}">
			<ul class = "adminfo row">
				<li><span>广告标题：</span><input type="hidden" name="adId" value="${ad.adId}" ><input maxlength="50" type="text" name="adName" value="${ad.adName}"><span class = "_star ">*</span></li>
				...
				<div id="img" style= "display:inline-block; float:left;">
					<div class="imgbox">
					    <div class="imgnum">
					        ...
					        <img src="<c:if test='${!empty ad.ad_thumb_pic1 and ad.ad_thumb_pic1!="" }'>${ad.ad_thumb_pic1 }</c:if>  <c:if test='${empty ad.ad_thumb_pic1 or ad.ad_thumb_pic1=="" }'>${ad.ad_thumb_pic1 }${ctx}/assets/img/btn.png</c:if>" class="img1" />
					        <img src="" class="img2" />
					    </div>
					</div>
				</div>
				...
				<%-- <li><span>大图：</span><img  src="${ad.ad_thumb_pic1}" width="120px" height="100px" alt=""><input type="file" name="file1" id="file1" onchange="previewImg(this)"></li>
				<li><span>小图：</span><img  src="${ad.ad_thumb_pic2}" width="120px" height="100px" alt=""><input type="file" name="file2" id="file2" onchange="previewImg(this)"></li> --%>
				<li><span>展示方式：</span>
				...
				<li><span>广告链接：</span>
				<input id="adId" type="text" name="adLinkurl" value="${ad.adLinkurl}" > 
				</li>
				<%-- <li><span>详情：</span><textarea style="width: 100%;min-height: 400px;resize:none; border: 1px solid #dbe2e6; border-radius: 4px; outline-color: #0099e0;" name="adInfo" id="myEditor" rows="" cols="">${ad.adInfo}</textarea></li> --%>
				<!-- 开始-->
				<li id="textArea" style="display: none;"><span>详情：</span><textarea style="width: 92%;min-height: 350px;resize:none; outline-color: #0099e0;" name="adInfo" id="myEditor" rows="" cols="">${ad.adInfo}</textarea></li>
				<li id="searchbatton" ><span>产品信息：</span>&nbsp;&nbsp;<input onclick="searchProduct(1)" style="width: 100px;text-indent: 0px; float:left;" class = "public_btn bg1" type="button" name="inquery" id="inquery" value = "查询商品"/></li>
				...
				<div id="prodInfo" class = "table-responsive m40">
					...
					<thead>
			       	...
			       	<c:forEach items="${productList}" var="product" varStatus="sta">
						<tr>
							<td><input type="hidden" name="adProdList[${sta.count-1}].catId" value="${product.catId}"><input type="hidden" name="adProdList[${sta.count-1}].prodId" value="${product.productId}"><input type="hidden" name="productId" value="${product.productId}"><input type="hidden" name="adProdList[${sta.count-1}].prod_code" value="${product.productCode}">${product.productCode}</td>
							<td><input type="hidden" name="adProdList[${sta.count-1}].prod_name" value="${product.productName}">${product.productName}</td>
							<td><input type="hidden" name="adProdList[${sta.count-1}].catName" value="${product.catName}">${product.catName}</td>
				...
				<!-- 结束 -->
				<li><span>排序：</span><input maxlength="3" type="text" name="sortOrder" value="${ad.sortOrder}" onfocus="InitInput.setNumber(this,3,0,0)"><span class = "_star ">*</span></li>
				<li style="display:flex;align-items:center">
				<span>广告状态：<input name = "enabled" id = "enabled" type="hidden" value="${ad.enabled}"></span>
				...
				<li><span>发布日期：</span><input onFocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" type="text" name="addTime" value="${ad.addTime}" ><span class = "_star ">*</span></li>
			</ul>
	       	<ul class = "adminfo row">
	       		<li>
					<span></span>
