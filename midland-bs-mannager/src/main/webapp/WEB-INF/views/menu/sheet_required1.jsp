...
<body>
            ...
            <li class=""> <span>地区：</span>
                ...
                <p id="sheet" style="display: inline-block;height: 28px;">
                    <span>片区：</span>
                    <input type="hidden" value="" name="sheetId">
                    <input type="hidden" value="" name="sheetName">
                    <select id="sheets" name="sheets" onchange="initSheet()" style="height: 100%;width: 102px; display: inline-table;border-radius: 4px;border: 1px solid #dbe2e6;">
                        <c:choose>
                            <c:when test="${not empty item.sheetId}">
                                <option value="${item.sheetId}">${item.sheetName}</option>
                            </c:when>
                            <c:otherwise>
                                <option value="">请选择</option>
                            </c:otherwise>
