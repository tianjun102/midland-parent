...
public class UserController extends CommonsController {
				...
				if (null != userInfo) {
					...
					if (userInfo.getPassword().equals(encryptedPwd)) {
						...
						if (userInfo.getUserType() == 2) {
							User catUser = userService.findParentUserByChild(userInfo.getUsername());
							if(catUser!=null){
	...
	public String toEdit(HttpServletRequest request, @RequestBody Map<String,String> params) {
		Result<User> result = new Result<>();
		User user = userService.getUserByUserId(Integer.valueOf(params.get("userId")));
	...
	public String installerList(@RequestBody Map<String, String> params,HttpServletRequest request) {
		...
		try {
			PageList<User>  items = userService.findBycreateByPage(params, new PageBounds());
	...
	public String installerDetail(@RequestBody Map<String, String> params) {
		...
		try {
			User  user =  userService.queryUser(Integer.valueOf(params.get("id")));
			result.setModel(user);
			result.setCode(ResultStatusUtils.STATUS_CODE_200);
