...
<div class="content">
	...
	<form class="login-form" action="rest/user/login" method="post">
		...
		<div class="form-group">
			...
			<div class="input-icon">
				...
				<input name="username" id="username" size="25" value="<%=username %>" class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="用户名" onblur="setUn();"/>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="input-icon">
				...
				<input name="password" id="password" size="25" value="<%=password %>" class="form-control placeholder-no-fix" type="password" autocomplete="off" placeholder="密码" />
			</div>
		</div>
		<div class="form-group">
			...
			<div class="input-icon">
				<i class="fa fa-lock"></i>
				<input type="text" name="id-code" id="id-code" placeholder="验证码" maxlength="4" onblur="validate();" /> 
				<a href="#" class="yz-code"><input type="button" id="code" onclick="createCode();" /></a> 
		...
		<div class="form-actions">
			...
			<input type="checkbox" name="remember" onclick="remember();"/> 记住我 </label>
	...
	<form class="forget-form" action="rest/user/sendMail" method="post">
		<h3>忘记密码 ?</h3>
		<p>
			 <!-- 请输入您的电子邮箱地址来重置您的密码. -->
			 我们已经发送了校验码到你的手机：
		...
		<input  type="hidden"  id="un" name="un" value=""/>
			<div class="input-icon">
				...
				手机号:<input type="text" id="mobile" name="mobile"/><button type="button" onclick="sendVcode();">获取验证码</button>
				验证码:<input type="text" id="vcode" name="vcode" onblur="checkVcode();"/>
...
//记住密码功能
function remember(){
var remFlag = $("input[name='remember']").is(':checked');
...
function checkVcode(){
	...
	$.ajax({ 
		type: "post", 
		url: "${ctx }/rest/user/checkVcode",
		async:false, // 此处必须同步
		dataType: "json",
		data:{"username":username,"mobile":mobile,"newPwd":newPwd},
		success: function(xmlobj){ 
			if (xmlobj.flag==1){
				a=true;
			}else{
